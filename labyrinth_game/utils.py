...
def random_event(game_state):
    """
    Создает небольшие случайные события во время перемещения игрока.
    ...
    if event_chance == 0:
        ...
        match event_type:
            case 0:
                # Сценарий 1 (Находка): Игрок находит на полу монетку
                print("Вы заметили что-то блестящее на полу...")
                print("Это монетка!")
                # Добавьте 'coin' в список предметов текущей комнаты
                current_room = constants.ROOMS[game_state['current_room']]
            ...
            case 1:
                ...
                # Если у игрока в инвентаре есть 'sword', 
                # выведите дополнительное сообщение
                if 'sword' in game_state['player_inventory']:
                    print("Вы достаете меч, и шорох мгновенно стихает.")
                else:
                    print("Шорох постепенно затихает.")
            
            case 2:
                # Сценарий 3 (Срабатывание ловушки): 
                # Проверьте, находится ли игрок в trap_room 
                # и отсутствует ли у него в инвентаре 'torch'
                if (game_state['current_room'] == 'trap_room' and 
                    'torch' not in game_state['player_inventory']):
                    # Если оба условия верны, выведите сообщение об опасности
                    print("В темноте вы не заметили ловушку!")
                    # Вызовите функцию trigger_trap()
                    trigger_trap(game_state)
                else:
